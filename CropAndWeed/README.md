...
- Practical full-stack experience: tying a CV model to a user-facing application.
- Production-aware engineering: env-based secrets, proxying, build scripts, and a small, testable API.
- UI/UX focus: polished, presentation-first interface designed for demos and interviews.

Include these bullets in your GitHub repo description and README summary to catch attention quickly.
...
- Add a small `docker-compose` setup to run backend + frontend locally with one command.
- Add GitHub Actions for CI (linting and build check) and optional deploy workflow.
- Redact API keys and add `.env.example` with required variables.
- Add more docs and screenshots to the README.
...
This repo is provided as-is for demo purposes. Add a license if you intend to publish publicly (MIT or Apache-2.0 are common choices).

Author: CropAndWeed demo — available for more features and deployment advice.
